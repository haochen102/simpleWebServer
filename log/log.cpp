...
void Log::write_log(int level, const char *format, ...) {
    ...
    //写入一个log，更新现有行数m_count
    m_mutex.lock();
    m_count++;

    //日志不是今天或写入的日志行数是最大行m_split_lines的倍数
    ...
    //写入内容格式：时间 + 内容
    //时间格式化，snprintf成功则返回写字符的总数，其中不包括结尾的null字符
    ...
    if (m_is_async && !m_log_queue->full()) {//采用异步，将日志信息加入阻塞队列
        m_log_queue->push(log_str);
    } else {//（默认）采用同步，加锁向文件中写
        m_mutex.lock();
